...
## 0. HWPX 가 한글에서 글자가 겹쳐 보임 (자간 + 변경 추적)

### 증상
한글에서 출력본 열면 일부 셀의 글자가 좌우로 겹치거나, 다른 텍스트가
원본 위에 덧씌워진 것처럼 보임.

### 두 가지 다른 원인 (혼동 주의)

#### 원인 A: 자간 자기-오염 (진짜 자간 버그)
- `section0.xml` 의 셀별 `<hp:linesegarray>` 의 `horzsize` 가 셀 폭과 안 맞음
- 빈 셀 폴백으로 들어간 `DEFAULT_LINESEG` 의 `horzsize="31508"` 이 다음 주
  합본을 다시 템플릿으로 사용하면서 셀에 그대로 살아남고 → 무한 자기 복제
- **검증법**: `python -c "import zipfile, re; ..."` 로 파일 안에
  `horzsize="31508"` 을 검색. 0 건이 정상
- **해결**: `extract_cell_lineseg()` 가 31508 만나면 같은 col 의 깨끗한
  lineseg 자동 차용. 커밋 `d11c499`

#### 원인 B: 한글의 변경 추적(Track Changes) 표시 모드
- 셀 폭이 정상이어도, 한글이 파일을 "변경 내용 표시" 보기 모드로 열면
  삽입 표시·취소선 등 편집 흔적이 원문 위에 겹쳐 그려짐
- `Contents/header.xml` 의 `<hh:trackchageConfig flags="56"/>` 가 트리거
- **확인법**: 한글에서 검토 → "변경 추적" 끄거나, 보기 모드를 "최종본"
  으로 전환 → 겹침이 사라지면 이 케이스
- **해결**: build_report() 가 header 의 flags 값을 0 으로 강제 설정.
  커밋 (이번 수정)
...
| 상황 | 원인 | 보기 |
|------|------|------|
| 모든 사람 셀에서 동일하게 좁아 보임 | A (자간 폭) | "최종본" 모드로 봐도 여전 |
| 특정 텍스트가 원문 위에 덧씌워짐 | B (변경 추적) | "최종본" 모드로 보면 정상 |
