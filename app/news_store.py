...
NEWS_SECTIONS = [
    ...
    ("3 배설", (("배설 케어 로봇", KO), ("배설 예측 센서", KO),
                ...
                ("排泄 支援 介護", JA))),
    # 4·5 유연착용형 + 인체영향성 (둘 다 한벼리) — **합쳤다**(2026-08).
    # 엑셀에서 두 과제가 **같은 행의 같은 키워드**라 억지로 갈라 놨더니 결과가
    # 양쪽에 똑같이 떴다(엔젤로보틱스 기사가 4·5 동시 게재). 한 탭이 맞다.
    # ⚠️ '외골격 로봇'은 0건(붙여 쓰면 안 잡힘) → '착용형 로봇'·'엑소슈트'.
    ("4·5 착용형", (("웨어러블 로봇", KO), ("착용형 로봇", KO),
                    ("엑소슈트", KO), ("소프트 외골격", KO),
                    ("보행 재활 로봇", KO), ("외골격 임상시험", KO),
    ...
    ("9 식사", (("식사보조로봇", KO), ("식사 지원 로봇", KO),
                ...
                ("食事 支援 ロボット", JA))),
    # 10·11 커뮤니케이션 로봇 + AI 챗봇 — **합쳤다**(2026-08).
    # 둘 다 '대화로 돌보는 것'이라 주제가 붙어 있고, 따로 두면 각각 자주 비었다
    # (10번은 하루 0~1건). 로봇(효돌·소셜로봇)과 비대면 서비스(AI 안부전화)를 함께 본다.
    # ⚠️ '고독사 예방'은 뺐다 — 뉴스에서는 지자체 복지행정만 물어와(하루 7건)
    #    로봇 기사가 통째로 묻혔다.
    # 11번 키워드 출처: `11. AI챗봇/AI챗봇_유사서비스_자료조사_202608.xlsx`(자체 조사).
    ("10·11 커뮤니케이션·AI챗봇",
     (("소셜로봇", KO), ("반려로봇 노인", KO), ("독거노인 돌봄 로봇", KO),
      ("말벗 로봇", KO), ("시니어 AI 챗봇", KO), ("AI 안부전화", KO),
