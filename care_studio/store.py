...
LOGBOOKS = [
    # 평가매뉴얼은 프로그램 종류마다 **주기가 다르다**(지표 24·25·26).
    # 한 대장으로 묶으면 사회적응(월 1회)이 신체기능 주기에 묻힌다 → 셋으로 나눈다.
    ("프로그램", "신체기능 프로그램 일지", "주3회",
     "지표 24 신체기능 프로그램 계획에 따라 주 3회 이상",
     [("프로그램명", "text"), ("참여인원", "num"), ("진행자", "staff"),
      ("내용·특이사항", "text")]),
    ("인지프로그램", "인지기능 프로그램 일지", "주3회",
     "지표 25 인지기능 프로그램 계획에 따라 주 3회 이상",
     [("프로그램명", "text"), ("참여인원", "num"), ("진행자", "staff"),
      ("내용·특이사항", "text")]),
    ("사회적응", "사회적응 프로그램 일지", "매월",
     "지표 26 사회적응 프로그램 계획에 따라 월 1회 이상",
     [("프로그램명", "text"), ("참여인원", "num"), ("진행자", "staff"),
      ("내용·특이사항", "text")]),
...
PROGRAM_KEYS = ["프로그램", "인지프로그램", "사회적응"]


def today_programs():
    """오늘 진행한 프로그램(세 대장 모두). 홈에 띄운다."""
    d = today()
    out = []
    for k in PROGRAM_KEYS:
        for r in logs(k):
            if r["날짜"] == d:
                r = dict(r)
                r["유형"] = LOG_SPEC[k][0].replace(" 프로그램 일지", "")
                out.append(r)
    return out
