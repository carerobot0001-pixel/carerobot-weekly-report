...
- **🏠 홈** (사이드바 첫 메뉴, 로그인 후 기본 화면): `home_page()`. 구성(위→아래):
  📌 공지사항 → 지표 5개 → ⚡**바로가기 타일 바**(네이버 아이콘식, 8개 메뉴 아이콘+라벨,
  `_goto`로 이동; `.qbar-mark` 컨테이너에 스코프된 CSS로 타일 스타일) → **2단 레이아웃**〔좌: 🔔오늘 챙길 것
  (주간보고 **화 17시 마감 카운트다운**·문서협업 마감) + 🙋내 할 일(이름 선택→미제출·협업 담당)
  / 우: 📅**일정 임베드 달력**(라디오로 주간/월간/일정목록 전환, 기본 주간) + 📰**뉴스 섹션 탭**〕
  → (하단 전체폭 토글) ➕일정 추가·수정·삭제(`_calendar_manage`). 지표/일정 읽기는 try/except로
  감싸 하나 실패해도 뜸. 홈 렌더 시에만 주입되는 **컴팩트 CSS**(폰트·여백 축소)로 화면 압축.
