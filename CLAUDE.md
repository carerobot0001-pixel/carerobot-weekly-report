...
- **🏠 홈** (사이드바 첫 메뉴, 로그인 후 기본 화면): `home_page()`. 구성(위→아래):
  ...
  - **뉴스 섹션 탭**(`news_store.NEWS_SECTIONS`): 돌봄로봇/돌봄·복지/AI·신기술/로봇·휴머노이드 4탭,
    ...
    - **섹션 구성은 매일 받는 뉴스 브리핑의 편집 지침을 따랐다**(2026-08, 5탭):
      돌봄로봇(로봇·보조기술) / 돌봄·정책(제도·인력·치매·재가) / AI·LLM(모델·오픈소스·
      에이전트·벤치마크) / AI 활용법(프롬프트·RAG·업무 자동화) / 로봇·휴머노이드.
      ⚠️ 키워드 검색은 지침의 **판단**("투자 헤드라인 말고 실질", "정말 새로운 기법인가",
      "왜 중요한가")을 못 한다. 맞출 수 있는 건 *주제 범위*까지다.
      키워드는 후보를 실제 조회해 수확이 좋은 것만 남겼다.
