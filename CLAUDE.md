...
- **파일을 안 받고 링크만 받는다.** 500곳 규모에서 파일 보관은 *버전 관리*(제조사가
  개정해도 우리 것은 옛날 판 → 현장에서 옛 매뉴얼 보고 기기를 만지면 사고)·저작권·
  삭제 요청이 전부 우리 일이 된다. 링크면 제조사가 갱신하는 순간 최신.
  (`collab_store`가 링크 방식인 것과 같은 판단)
...
- 검증한 것: 탭 자동 생성·헤더·제출→`대기` 저장·행 삭제까지 로컬에서 실제로 확인.
