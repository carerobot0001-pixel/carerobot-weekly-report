...
- **🏠 홈** (사이드바 첫 메뉴, 로그인 후 기본 화면): `home_page()`. 새 저장소 없음 —
  기존 store들을 읽어 지표 5개(이번주 보고 X/10, 미해결 관리대장 문제, 대기 구매요청,
  진행중 협업, 등록 장비)와 **리마인더**(주간보고 미제출자, 문서협업 마감 임박/지남)를 표시.
  각 읽기는 try/except로 감싸 하나 실패해도 대시보드는 뜸(값은 '—').
