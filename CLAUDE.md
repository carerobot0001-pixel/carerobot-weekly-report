...
- **설계 원칙(Claude 데스크탑 방식)**: 배경은 **중성 회색**(`#262624`, 면은 `#30302e`),
  글씨는 **흰색**(본문 `#f0efeb`), 주황은 **포인트에만**(주요버튼·아이콘·링크).
  ⚠️ 눈부심의 원인은 글씨 밝기가 아니라 **채도 높은 주황이 곳곳에 있는 것** — 글씨를
  어둡게 낮추면 오히려 안 보인다(실제로 한 번 그렇게 했다가 되돌림).
