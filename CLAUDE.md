...
"메일은 보냈고 상대 답을 기다리는 중"인 일. **✓(완료)를 누르면 실적 기록이 되어버리는데
아직 끝난 게 아니고, 그냥 두면 손도 안 댄 일처럼 보인다.** 그 사이를 메운다.

- 시트: `TODO_HEADER` **맨 뒤에 `대기` 추가**(옛 행 안 밀림). 값은 *기다리기 시작한 날짜*
  `YYYY-MM-DD` — 며칠째인지 세기 위해. `todo_store.set_wait(uid,row,text,on)`.
  ⚠️ 컬럼을 늘렸으면 `append_row([...])` **3곳**(`set_sync`·`add_todo`·`complete_todo`)의
  칸 수도 같이 늘려야 한다. 안 그러면 마지막 칸이 비는 게 아니라 헤더와 어긋난다.
- 화면: 항목 옆 `⋯` → 켜지면 `↩`. 배지 `회신대기 N일`(7일↑ 붉게), 정렬은 **아래로**
  (`_todo_sort_key`의 첫 값 `1+wait`) — 사라지진 않는다. 드래그로 순서를 준 항목은
  그 순서가 우선.
- 기호는 **글자**(`⋯`·`↩`)를 쓴다 — 이모지는 CSS 색이 안 먹어 다크모드에서 흰 박스가 된다.
  버튼 키 `todo_wait_`를 작은버튼 높이 CSS와 다크모드 색 규칙 **양쪽에** 넣어야 한다.
