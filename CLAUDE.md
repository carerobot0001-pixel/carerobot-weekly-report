...
- **🏠 홈** (사이드바 첫 메뉴, 로그인 후 기본 화면): `home_page()`. 구성:
  📌 공지사항(`notice_store`, 담당자만 등록/삭제) → 지표 5개 → ⚡빠른실행 버튼(_goto로 각
  페이지 이동) → 🔔오늘 챙길 것(주간보고 **화 17시 마감 카운트다운**·문서협업 마감) →
  🙋내 할 일(이름 선택→미제출·협업 담당) → 📌오늘 일정(캘린더) → 🗓️캘린더 임베드 →
  📰 뉴스(`news_store`). 지표/일정 읽기는 try/except로 감싸 하나 실패해도 뜸.
  버튼 네비게이션은 사이드바 radio에 `key="main_menu"` + `_nav_to`(_goto)로 구현.
